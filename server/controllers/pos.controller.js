...
export const createSale = async (req, res) => {
  try {
    ...
    for (const item of items) {
      ...
      if (medicine.Mquantity < item.quantity) {
        return res
          ...
          .json({
            success: false,
            message: `Insufficient stock for ${medicine.Mname}. Available: ${medicine.Mquantity}`,
          });
      }
    ...
    // Generate unique invoice number: INV-YYYYMMDD-NNNN
  ...
  try {
    ...
    const totalProfit = todaySales.reduce((sum, sale) => {
      const cost = sale.items.reduce(
        (c, item) => c + item.costPrice * item.quantity,
        0,
      );
...
export const getAnalytics = async (req, res) => {
  try {
    ...
    sales.forEach((sale) => {
      ...
      dailyMap[day].items += sale.items.reduce(
  ...
  try {
    ...
    if (/^\d{8,13}$/.test(query) || query.startsWith("KMP")) {
      results = await Inventory.find({
        $or: [{ factoryBarcode: query }, { generatedBarcode: query }],
        status: { $ne: "Expired" },
      }).limit(5);
    }
...
export const getExpiryAlerts = async (req, res) => {
  try {
    ...
    const alreadyExpired = await Inventory.find({ expirAt: { $lt: now } }).sort(
      { expirAt: 1 },
    );
    const expiring30 = await Inventory.find({
      expirAt: { $gte: now, $lte: days30 },
    }).sort({ expirAt: 1 });
    const expiring60 = await Inventory.find({
      expirAt: { $gt: days30, $lte: days60 },
    }).sort({ expirAt: 1 });
    const expiring90 = await Inventory.find({
      expirAt: { $gt: days60, $lte: days90 },
    }).sort({ expirAt: 1 });

    const expiredValue = alreadyExpired.reduce(
      (sum, item) => sum + item.Mprice * item.Mquantity,
      0,
    );
    const atRiskValue = expiring30.reduce(
      (sum, item) => sum + item.Mprice * item.Mquantity,
      0,
    );
...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const totalInventoryValue = await Inventory.aggregate([
      {
        $group: {
          _id: null,
          total: { $sum: { $multiply: ["$Mprice", "$Mquantity"] } },
        },
      },
    ]);
    ...
    const expiringCount = await Inventory.countDocuments({
      expirAt: { $gte: new Date(), $lte: days30 },
    });
