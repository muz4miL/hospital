...
import { FiSearch, FiTrash2, FiPlus, FiMinus, FiPrinter, FiShoppingCart, FiX } from 'react-icons/fi';
...
export default function POSPage() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      const isOtherInput = activeEl && (activeEl.tagName === 'INPUT' || activeEl.tagName === 'TEXTAREA') && !isSearchInput;
      if (isOtherInput) return;

      // F-key shortcuts
      if (e.key === 'F1') { e.preventDefault(); handleNewSale(); return; }
      if (e.key === 'F2') { e.preventDefault(); searchRef.current?.focus(); return; }
      if (e.key === 'F12') { e.preventDefault(); handleCheckout(); return; }
      if (e.key === 'Escape') { e.preventDefault(); setSearchResults([]); setSearchTerm(''); setShowReceipt(false); return; }
  ...
  const handleCheckout = async () => {
    ...
    try {
      const res = await fetch('/api/pos/sale', {
        ...
        body: JSON.stringify({
          items: cart.map(({ medicineId, medicineName, quantity }) => ({ medicineId, medicineName, quantity })),
          discount,
          discountType,
        ...
        <div className="flex-1 flex flex-col p-4 overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-2 text-xs">
              <kbd className="px-2 py-1 bg-gray-700 text-gray-300 rounded border border-gray-600">F1</kbd>
              <span className="text-gray-400">New</span>
              <kbd className="px-2 py-1 bg-gray-700 text-gray-300 rounded border border-gray-600">F2</kbd>
              <span className="text-gray-400">Search</span>
              <kbd className="px-2 py-1 bg-gray-700 text-gray-300 rounded border border-gray-600">F12</kbd>
              <span className="text-gray-400">Checkout</span>
              <kbd className="px-2 py-1 bg-gray-700 text-gray-300 rounded border border-gray-600">Esc</kbd>
          ...
          <div className="relative mb-3">
            ...
            />
            {searchTerm && (
              <button onClick={() => { setSearchTerm(''); setSearchResults([]); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white">
                <FiX />
              </button>
            )}
                ...
                {searchResults.map((medicine, i) => (
                  <div
                    ...
                    className={`flex items-center justify-between px-4 py-3 cursor-pointer transition-colors ${
                      i === selectedIndex ? 'bg-emerald-600' : 'hover:bg-gray-700'
                    }`}
                  >
                    <div>
                      <p className={`font-medium ${i === selectedIndex ? 'text-white' : 'text-gray-200'}`}>{medicine.Mname}</p>
                      <p className="text-xs text-gray-400">{medicine.type} • {medicine.Msupplier} • Stock: {medicine.Mquantity}</p>
                    </div>
                    <div className="text-right">
                      <p className={`font-bold ${i === selectedIndex ? 'text-white' : 'text-emerald-400'}`}>Rs. {medicine.Mprice.toLocaleString()}</p>
                      <p className="text-xs text-gray-400">{medicine.unit || 'Strip'}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex-1 overflow-y-auto bg-gray-800 rounded-lg border border-gray-700">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-500">
                ...
                <p className="text-sm mt-1">Type medicine name or scan barcode to add</p>
              </div>
            ) : (
              <table className="w-full">
                <thead className="bg-gray-750 sticky top-0">
                  <tr className="text-gray-400 text-sm border-b border-gray-700">
                    ...
                    <th className="text-left px-4 py-2 bg-gray-800">Medicine</th>
                ...
                <tbody>
                  {cart.map((item, i) => (
                    <tr key={item.medicineId} className="border-b border-gray-700 hover:bg-gray-750 transition-colors">
                      <td className="px-4 py-3 text-gray-500 text-sm">{i + 1}</td>
                      <td className="px-4 py-3">
                        <p className="text-white font-medium">{item.medicineName}</p>
                        <p className="text-xs text-gray-500">Stock: {item.maxStock} {item.unit}</p>
                      </td>
                      <td className="px-4 py-3 text-center text-gray-300">Rs. {item.unitPrice.toLocaleString()}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => updateQuantity(item.medicineId, item.quantity - 1)} className="p-1 rounded bg-gray-700 hover:bg-gray-600 text-gray-300">
                            <FiMinus size={14} />
                          </button>
                          <input
                            type="number"
                      ...
                      <td className="px-4 py-3 text-right text-emerald-400 font-bold">Rs. {item.total.toLocaleString()}</td>
                      <td className="px-4 py-3 text-center">
                        <button onClick={() => removeFromCart(item.medicineId)} className="p-1.5 rounded hover:bg-red-500/20 text-red-400 hover:text-red-300 transition-colors">
                          <FiTrash2 size={16} />
                        </button>
                      </td>
                    </tr>
        ...
        <div className="w-80 bg-gray-800 p-4 flex flex-col border-l border-gray-700 overflow-y-auto">
          {!showReceipt ? (
            <>
              {/* Customer Info */}
              <div className="mb-4">
                <label className="text-xs text-gray-400 uppercase tracking-wider">Customer (Optional)</label>
                <input
                  type="text"
                  value={customerName}
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {[
                    ...
                    { value: 'Card', icon: <BsCreditCard2Back />, label: 'Card' },
                    { value: 'Mobile', icon: <MdPhoneAndroid />, label: 'JazzCash' },
                    { value: 'Credit', icon: <HiOutlineReceiptPercent />, label: 'Udhar' },
                  ].map(method => (
                    <button
                      key={method.value}
              ...
              {/* Amount Paid */}
              <div className="mb-4">
                <label className="text-xs text-gray-400 uppercase tracking-wider">Amount Paid</label>
                <input
                  type="number"
                  value={amountPaid}
            ...
            <div className="flex flex-col h-full">
              <div className="text-center mb-4">
                ...
                <p className="text-gray-400 text-sm mt-1">{lastSale?.invoiceNumber}</p>
              </div>
              ...
              <div className="bg-gray-900 rounded-lg p-4 mb-4 space-y-2">
                ...
                <div className="flex justify-between text-white font-bold text-lg pt-2 border-t border-gray-700">
                  <span>Total</span>
                  <span className="text-emerald-400">Rs. {lastSale?.grandTotal.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-gray-400 text-sm">
              ...
              <div className="mt-auto space-y-2">
                <button onClick={printReceipt} className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg flex items-center justify-center gap-2 transition-colors">
                  <FiPrinter /> Print Receipt
                </button>
                <button onClick={handleNewSale} className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg transition-colors">
                  New Sale (F1)
                </button>
              </div>
            </div>
