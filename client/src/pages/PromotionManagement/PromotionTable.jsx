...
function PromotionTable() {
    ...
    const fetchData = async () => {
        try {
            const fetchPromotion = await axios.get('/api/promotion/read');
            ...
            const updatedPromotions = response.promotion.map(promo => {
                if (new Date(promo.expiredAt) < new Date()) {
                    ...
                    axios.put(`/api/promotion/update/${promo._id}`, { status: 'Inactive' })
                    .then(response => {
    ...
    const handleDeleteConfirmed = async () => {
        try {
            await axios.delete(`/api/promotion/delete/${deleteId}`);
            setData(prevState => ({
                ...prevState,
        ...
        <div>
            <form  className='px-10 py-2 pb-7 flex justify-end' onSubmit={handleSearch}>
                <div className='relative'>
                    <input type='text' placeholder='Search Promotions' className='bg-gray-800 border border-gray-700 rounded-md text-white placeholder-gray-500 focus:outline-none w-56 p-2 pl-10' onChange={(e) => setSearchQuery(e.target.value)} value={searchQuery}/>
        ...
        <table className="w-full border border-gray-700">
            <thead>
                <tr className="bg-gray-800 text-white text-left">
                    <th className="border border-gray-700 px-4 py-2">Promotion ID</th>
                    <th className="border border-gray-700 px-4 py-2">Coupon Code</th>
                    <th className="border border-gray-700 px-4 py-2">Coupon Price</th>
                    ...
                    <th className="border border-gray-700 px-4 py-2">Expired At</th>
            ...
            <tbody>
                {searchResults?.map((elem, index) => {
                    ...
                    <tr key={index} className="bg-gray-900 text-gray-300">
                        <td className="border-b border-gray-700 px-4 py-2">{elem.promotionID}</td>
                        <td className="border-b border-gray-700 px-4 py-2">{elem.couponCode}</td>
                        <td className="border-b border-gray-700 px-4 py-2">{elem.couponPrice}</td>
                        <td className="border-b border-gray-700 px-4 py-2">{elem.totalAmount}</td>
                        <td className="border-b border-gray-700 px-4 py-2">{elem.type}</td>
                        <td className="border-b border-gray-700 px-4 py-2">{formatDate(elem.createdAt)}</td>
                        <td className="border-b border-gray-700 px-4 py-2">{formatDate(elem.expiredAt)}</td>
