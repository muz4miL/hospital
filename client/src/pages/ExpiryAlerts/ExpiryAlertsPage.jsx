...
export default function ExpiryAlertsPage() {
  ...
  const getFilteredMedicines = () => {
    ...
    if (activeTab === "all") {
      ...
      medicines = (data.expired || []).map((m) => ({
        ...m,
        alertType: "expired",
      }));
    } else if (activeTab === "30days") {
      medicines = (data.expiring30 || []).map((m) => ({
        ...m,
        alertType: "30days",
      }));
    } else if (activeTab === "60days") {
      medicines = (data.expiring60 || []).map((m) => ({
        ...m,
        alertType: "60days",
      }));
    } else if (activeTab === "90days") {
      medicines = (data.expiring90 || []).map((m) => ({
        ...m,
        alertType: "90days",
      }));
    }
  ...
  const exportCSV = () => {
    ...
    const rows = medicines.map((m) => [
      ...
      m.Msupplier,
      m.batchNumber || "-",
      formatDate(m.expDate),
      getDaysUntilExpiry(m.expDate),
      m.Mquantity,
      m.Mprice,
      (m.Mquantity * m.Mprice).toFixed(0),
      m.alertType,
    ]);
  ...
  const totalValue = medicines.reduce(
    (sum, m) => sum + m.Mquantity * m.Mprice,
    0,
  );
      ...
      <div className="flex-1 p-6 overflow-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">
              Monitor medicine expiry dates and take action
            </p>
          </div>
          <button
            onClick={exportCSV}
            className="flex items-center gap-2 bg-zinc-800 text-zinc-300 px-4 py-2 rounded-lg hover:bg-zinc-700 transition text-sm"
          >
            ...
            <div className="bg-zinc-900 rounded-xl p-4 border border-zinc-800">
              ...
              <p className="text-2xl font-bold text-white mt-1">
                {(data.summary?.expiredCount || 0) +
                  (data.summary?.expiring30Count || 0) +
                  (data.summary?.expiring60Count || 0) +
                  (data.summary?.expiring90Count || 0)}
              </p>
              <p className="text-zinc-600 text-xs mt-1">medicines</p>
            </div>
            <div className="bg-zinc-900 rounded-xl p-4 border border-red-900/50">
              ...
              <p className="text-zinc-600 text-xs mt-1">
                PKR {(data.summary?.expiredValue || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-zinc-900 rounded-xl p-4 border border-orange-900/50">
              ...
              <p className="text-zinc-600 text-xs mt-1">
                PKR {(data.summary?.expiring30Value || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-zinc-900 rounded-xl p-4 border border-yellow-900/50">
              <p className="text-yellow-400 text-xs uppercase tracking-wide font-medium">
                Within 60 Days
              </p>
              ...
              <p className="text-zinc-600 text-xs mt-1">
                PKR {(data.summary?.expiring60Value || 0).toLocaleString()}
              </p>
            </div>
            <div className="bg-zinc-900 rounded-xl p-4 border border-blue-900/50">
              ...
              <p className="text-zinc-600 text-xs mt-1">
                PKR {(data.summary?.expiring90Value || 0).toLocaleString()}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
          ...
          <div className="relative">
            <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-500 text-sm" />
            <input
              type="text"
              placeholder="Search medicine, supplier, batch..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="bg-zinc-900 border border-zinc-800 text-white text-sm rounded-lg pl-9 pr-4 py-2 w-72 focus:outline-none focus:border-zinc-600"
            />
          </div>
        </div>
        ...
        {/* Total value bar */}
        {medicines.length > 0 && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-3 mb-4 flex items-center justify-between">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-zinc-800">
                    ...
                    <th className="text-center text-xs font-medium text-zinc-500 uppercase tracking-wide px-4 py-3">
                      Status
                    </th>
                  </tr>
                </thead>
                  ...
                  {medicines.length === 0 ? (
                    ...
                    medicines.map((m, idx) => {
                      ...
                      return (
                        <tr
                          key={idx}
                          className={`hover:bg-zinc-800/50 transition ${m.alertType === "expired" ? "bg-red-500/5" : ""}`}
                        >
                          ...
                          <td className="px-4 py-3 text-right text-zinc-300 text-sm">
                            PKR {m.Mprice?.toLocaleString()}
                          </td>
                          <td className="px-4 py-3 text-right text-zinc-300 text-sm font-medium">
                            PKR {(m.Mquantity * m.Mprice).toLocaleString()}
                          </td>
                          <td className="px-4 py-3 text-center">
                            {getAlertBadge(m.alertType)}
                          </td>
                        </tr>
                      );
                    })
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-zinc-600 text-xs">
                  Remove from inventory immediately. Contact supplier for
                  returns if within return window.
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-zinc-600 text-xs">
                  Prioritize selling these first. Consider offering small
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-zinc-600 text-xs">
                  Monitor closely. Don't reorder these items until current stock
                  is sold.
                </p>
              </div>
            </div>
