...
function EmployeeTable() {
    ...
    const fetchData = async () => {
        try {
            const fetchEmployee = await axios.get('http://localhost:3000/api/employeeSalary/read');
    ...
    const handleDeleteConfirmed = async () => {
        try {
            await axios.delete(`http://localhost:3000/api/employeeSalary/delete/${deleteId}`);
            setData(prevState => ({
                ...prevState,
        ...
        <table className="w-full border border-gray-700">
            <thead>
                <tr className="bg-gray-800 text-white text-left">
                    ...
                    <th className="border border-gray-700 px-4 py-2">Salary</th>
                    ...
                    <tr key={index} className="bg-gray-900 text-gray-300">
                        ...
                        <td className="border-b border-gray-700 px-4 py-2">
                            <div className='flex text-sm px-full'>
                                <Link to={`/update-Salary-employee/${elem._id}`}><button className='bg-green-600 text-white hover:bg-green-700 transition-all rounded  px-4 py-1'>Update</button></Link>
        ...
        {deleteId && (
                <div className="fixed top-0 left-0 flex items-center justify-center w-full h-full bg-black bg-opacity-80">
                    <div className="bg-gray-800 text-white p-8 rounded-lg shadow-lg">
                        <p className="text-lg font-semibold mb-4">Are you sure you want to delete this salary assignment?</p>
